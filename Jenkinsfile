  ...
  stages {
    ...
    stage('Deploy') {
      steps {
        ...
        sh "docker run -v /var/run/docker.sock:/var/run/docker.sock -d --restart=always --network=host --name '${IMAGE_TAG}' -e NODE_ENV=test -e VIRTUAL_HOST=${IMAGE_TAG}.mustdash.es -p 3001:3001 ${IMAGE_TAG}"
