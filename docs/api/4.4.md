...
To upload, send a `POST` request with a `Content-Type` or `multipart/form-data` to the `/upload` endpoint of the media bucket you wish to upload to. On successful upload, the file's metadata is returned as JSON, including an identifier that can be used to create a reference to the file from another collection.
