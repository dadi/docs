...
| `auth.saltRounds` | The difficulty factor used when hashing client secrets (lower values mean hashes are faster to generate but easier to crack) | *N/A* | `10` | Number
...
While the above configuration changes should be enough to get the application started, there are several more changes you should know about. They can be found in detail in the release notes for [API Version 3.0](https://github.com/dadi/api/releases/tag/v3.0.0).
