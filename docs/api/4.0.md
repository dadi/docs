...
<!--TODO: create clients via Clients API-->
...
- `roleStore`: stores API [roles](#) <!--TODO: replace URL-->
...
|--------|----------|----------|
| `create` | *N/A* | *N/A* |
| `delete` | *N/A* | Which documents can be deleted |
| `deleteOwn` | *N/A* | Which documents can be deleted |
| `read` | Which fields will be displayed | Which documents can be read |
| `readOwn` | Which fields will be displayed | Which documents can be read |
| `update` | Which fields can be updated | Which documents can be updated |
| `updateOwn` | Which fields can be updated | Which documents can be updated |
...
```http
POST /api/clients/testClient/resources HTTP/1.1
Content-Type: application/json
Authorization: Bearer c389340b-718f-4eed-8e8e-3400a1c6cd5a
...
At this point, `testClient` can [request an access token](#obtaining-an-access-token) and access the `library/book` collection.
...
API 3.0 comes with various performance and flexibility enhancements, some of which introduce breaking changes. This document is a step-by-step guide that will show you in detail how you can make your application ready for the upgrade.
...
When migrating from 2.0, we need to actively specify MongoDB as our database engine. Here's how.

1. Add `@dadi/api-mongodb` as a project dependency.
    
    ```shell
    npm install @dadi/api-mongodb --save
    ```
    
2. Create a MongoDB configuration file for each of the environments your application runs on. For example, if you currently have a `config.development.json` and `config.production.json` configuration files, you'll need to place `mongodb.development.json` and `mongodb.production.json` in the same directory.

    The contents of each MongoDB configuration file will be the value of the `database` field of the respective API configuration file, with a few modifications.

3. Each block of database overrides becomes namespaced under a `databases` block:
    ...
    *Before (`config.development.json`)*
    ...
    ```
    *After (`mongodb.development.json`)*
    ```json
    {
      "hosts": [
        {
...
4. In the API configuration files, add a `datastore` property both at root level with the value `@dadi/api-mongodb`.

5. Your API configuration files should have an `auth` block with a `database` object. Change this to be just a string with the name of the database you want to use for authentication, and add a `datastore` property with the value `@dadi/api-mongodb`.
    ...
    *After (`config.development.json`)*
    ...
    If the `dadiapiauth` database uses a set of hosts different from the default, ensure the `databases` block in `mongodb.development.json` has a key for it.
