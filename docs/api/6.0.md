...
| `datastore` | The name of the npm module that implements the data connector used for storing documents | *N/A* | `@dadi/api-mongodb` | String
| `auth.tokenUrl` | The endpoint used for token generation | *N/A* | `/token` | String
| `auth.tokenTtl` | The amount of time (in seconds) which bearer tokens are valid for | *N/A* | `1800` | Number
| `auth.tokenKey` | The private key used to sign JWT tokens | `TOKEN_KEY` | `` | String
| `auth.accessCollection` | The name of the internal collection used to store aggregate permissions data | *N/A* | `accessStore` | String
| `auth.clientCollection` | The name of the internal collection used to store clients | *N/A* | `clientStore` | String
| `auth.keyAccessCollection` | The name of the internal collection used to store aggregate permissions data for keys | *N/A* | `keyAccessStore` | String
| `auth.keyCollection` | The name of the internal collection used to store access keys | *N/A* | `keyStore` | String
| `auth.roleCollection` | The name of the internal collection used to store roles | *N/A* | `roleStore` | String
| `auth.datastore` | The name of the npm module that implements the data connector used for authentication | *N/A* | `@dadi/api-mongodb` | String
| `auth.database` | The name of the database used to store authentication data | `DB_AUTH_NAME` | `test` | String
| `auth.hashSecrets` | Whether to hash client secrets | *N/A* | `true` | Boolean
| `auth.saltRounds` | The number of rounds to go through when hashing a password | *N/A* | `10` | Number
| `search.database` | The name of the database to use for storing and querying indexed documents | `DB_SEARCH_NAME` | `search` | String
| `search.datastore` | The datastore to use for storing and querying indexed documents | *N/A* | `` | String
| `search.enabled` | If true, API responds to collection /search endpoints | *N/A* | `` | Boolean
| `search.indexCollection` | The name of the datastore collection that will hold the index of word matches | *N/A* | `searchIndex` | String
| `search.minQueryLength` | Minimum search string length | *N/A* | `3` | Number
| `search.wordCollection` | The name of the datastore collection that will hold tokenized words | *N/A* | `searchWords` | String
| `caching.ttl` | — | *N/A* | `300` | Number
...
| `caching.directory.autoFlush` | — | *N/A* | `true` | Boolean
| `caching.directory.autoFlushInterval` | — | *N/A* | `60` | Number
...
| `media.s3.accessKey` | The access key used to connect to an S3-compatible storage provider | `AWS_S3_ACCESS_KEY` | `` | String
| `media.s3.secretKey` | The secret key used to connect to an S3-compatible storage provider | `AWS_S3_SECRET_KEY` | `` | String
| `media.s3.bucketName` | The name of the S3 bucket in which to store uploads | `AWS_S3_BUCKET_NAME` | `` | String
| `media.s3.region` | The region for an S3-compatible storage provider | `AWS_S3_REGION` | `` | String
| `media.s3.endpoint` | The endpoint for an S3-compatible storage provider | *N/A* | `` | String
| `media.publicUrl` | The base URL where media assets can be publicly accessed at | `MEDIA_PUBLIC_URL` | `` | String
...
| `cors` | If true, responses will include headers for cross-domain resource sharing | *N/A* | `true` | Boolean
...
| `featureQuery.enabled` | Whether feature query via custom headers is enabled | *N/A* | `true` | Boolean
| `workQueue.debounceTime` | The amount of idle time (in ms) required for the work queue to start a background job | *N/A* | `500` | Number
| `workQueue.pollingTime` | The interval (in ms) at which the work queue checks for new background jobs | *N/A* | `200` | Number
| `schemas.collection` | The name of the internal collection to store collection schemas | *N/A* | `schemas` | String
| `schemas.loadSeeds` | Whether to scan the workspace directory for collection seed files | *N/A* | `true` | Boolean
...
| `collection:{PROPERTY}_{NAME}` | Access to the collection named `NAME` in the property `PROPERTY` | `collection:library_book` |
...
|--------|----------|----------|
| `create` | Controls which fields can be created | *N/A* |
...
paths:
  /api/clients:
              ...
              properties:
                ...
                data:
                  ...
                  example:
                    firstName: "Eduardo"
                    lastName: "Bouças"
                  description: An arbitrary data object to be associated with the client
                resources:
                  description: A list of resources to assign to the client
                  $ref: "#/components/schemas/ResourceWithAccessMatrix"
      responses:
        201:
    ...
    get:
      ...
      description: "Returns an array of all the client records. The requesting client must have `read` access to the `clients` resource, or have an `accessType` of `admin`."
      responses:
        200:
  ...
  /api/clients/{clientId}:
    get:
      ...
      description: "Finds a client with the given ID and returns it if one exists.. "
      parameters:
      - name: "clientId"
        in: "path"
    ...
    put:
      ...
      description: "Updates a client. For a client to update a record other than their own, they must have an `accessType` of `admin`. It is not possible to change a client ID, as it is immutable. It is also not possible to change any resources or roles using this endpoint – the resources and roles endpoints should be used for that effect. For a non-admin client to update their own secret, they must include the current secret in the request payload via the `currentSecret` property."
      parameters:
      - name: "clientId"
        in: "path"
        ...
        content:
          application/json:
            schema:
              ...
              properties:
                data:
                  description: An optional, arbitrary data object
                  type: object
                  example:
                    firstName: "Eduardo"
                currentSecret:
                  description: The current client secret, required whenever the `secret` property is supplied
                  type: string
                  example: "current-secret"
                secret:
                  description: The new client secret, if it is to be updated
                  type: string
                  example: "new-secret"
      responses:
        200:
    ...
    delete:
      ...
      description: "Deletes a client with the ID supplied, if one exists."
      parameters:
      - in: "path"
        name: "clientId"
      ...
      responses:
        204:
          description: "Successful operation; the client is deleted and no data is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  ...
  /api/client:
    get:
      ...
      description: "Returns information about the client associated with the bearer token present in the request."
      responses:
        200:
          description: "Successful operation; the client is returned with the secret omitted. Response includes the roles granted and resources they have access to."
          content:
  ...
  /api/clients/{clientId}/roles:
    post:
      ...
      description: "Assigns one or more roles to a client identified by the ID supplied."    
      parameters:
      - in: "path"
        name: "clientId"
        description: "The ID of the client to assign roles to"
        example: "eduardo"
        required: true
        schema:
          type: string
      requestBody:
        content:
          application/json:
            description: An array with the names of the roles to be assigned
            schema:
              type: array
              example:
                - employee
  ...
  /api/clients/{clientId}/roles/{roleName}:
    delete:
      tags:
      - "client"
      summary: "Unassign role from client"
      description: "Unassigns a role from a client identified by the ID supplied"
      parameters:
      - in: "path"
        name: "clientId"
      ...
      responses:
        204:
          description: "Successful operation; the role has been unassigned and no data is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  ...
  /api/clients/{clientId}/resources:
    post:
      tags:
      - "client"
      summary: "Assign a resource to a client"
      description: "Gives a client permissions to access a resource."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The ID of the client"
            ...
            schema:
              ...
              properties:
                name:
                  ...
                  example: "collection:library_book"
                access:
                  description: Access matrix for the resource
                  required: true
  ...
  /api/clients/{clientId}/resources/{resourceId}:
    delete:
      tags:
      - "client"
      summary: Revoke a resource from a client
      description: "Remove a client's permissions for the specified resource."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The ID of the clien"
        required: true
        schema:
          type: string
        example: "eduardo"
      - in: "path"
        ...
        schema:
          type: string
        example: "collection:library_books"
      responses:
        204:
    ...
    put:
      tags:
      - "client"
      summary: "Update a client's resource permissions"
      description: "Updates a client's permissions for accessing the resource."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The client to update resources on"
        required: true
        schema:
          type: string
        example: "eduardo"
      ...
      properties:
        ...
        roles:
          ...
          items:
            type: string
            example: "manager"
    ResourceWithAccessMatrix:
      type: "object"
      properties:
        "collection:library_book":
          $ref: "#/components/schemas/AccessMatrix"
    AccessMatrix:
      description: An object mapping access types to either a Boolean (granting or revoking that access type) or an object specifying field-level permissions and/or permission filters
      type: "object"
      properties:
        create:
          description: Permissions to create new entries
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        delete:
          description: Permissions to delete entries
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        deleteOwn:
          description: Permissions to delete entries created by the requesting user
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        read:
          description: Permissions to read entries
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        readOwn:
          description: Permissions to read entries created by the requesting user
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        update:
          description: Permissions to update entries
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
        updateOwn:
          description: Permissions to update entries created by the requesting user
          oneOf:
            - $ref: "#/components/schemas/AccessMatrixBoolean"
            - $ref: "#/components/schemas/AccessMatrixObject"
      example:
...
paths:
  /api/roles:
    post:
      ...
      description: "Creates a new role, which may be configured to extend an existing role."
      requestBody:
        content:
          application/json:
            schema:
              ...
              properties:
                name: 
                  ...
                  example: "manager"
                extends:
                  description: Name of role to extend
                  type: string
                  example: "employee"
                resources:
                  description: List of resources and corresponding permissions to assign to role
                  $ref: "#/components/schemas/ResourceWithAccessMatrix"                  
      responses:
        201:
          description: "Successful operation; the role has been created and its details are returned"
          content:
            application/json:
              schema:
                type: object
  ...
  /api/roles/{roleName}:
    get:
      ...
      description: "Returns a single role with the name supplied, if one exists."
      parameters:
      - in: "path"
        name: "roleName"
        description: "The name of the role"
      ...
      responses:
        200:
          description: "Successful operation; the role is found and its details returned"
          content:
            application/json:
              schema:
                type: object
    ...
    put:
      ...
      description: "Updates an existing role with the given name."
      parameters:
      - in: "path"
        name: "roleName"
        description: "The name of the role"
          ...
          application/json:
            schema:
              ...
              properties:
                extends: 
                  type: string
                  description: Name of a role to extend – if set to `null`, any relationship will be removed
                  example: "employee"
      responses:
        200:
          description: "Successful operation; the role has been updated and its new state is returned"
          content:
            application/json:
              schema:
                type: object
    ...
    delete:
      ...
      description: Deletes an existing role with the given name.
      parameters:
      - in: "path"
        name: "roleName"
      ...
      responses:
        204:
          description: "Successful operation; the role has been deleted and no data is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  ...
  /api/roles/{roleName}/resources:
    post:
      ...
      description: "Gives an existing role permissions to access a resource."
      parameters:
      - in: "path"
        name: "roleName"
        description: "The name of the role"
      ...
      responses:
        200:
          description: "Successful operation; the resource has been assigned to the role and the new state of the role is returned"
          content:
            application/json:
              schema:
                type: object
  ...
  /api/roles/{roleName}/resources/{resourceId}:
    delete:
      ...
      description: "Removes an existing role's permissions for accessing the specified resource"
      parameters:
      - in: "path"
        name: "roleName"
        description: "Name of the role"
      ...
      responses:
        204:
          description: "Successful operation; access revoked successfully and the updated role is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
    ...
    put:
      tags:
      - "role"
      summary: "Update a role's resource permissions"
      description: "Updates a role's permissions for accessing the resource."
      parameters:
      - in: "path"
        name: "roleName"
        description: "Name of the role"
      ...
      responses:
        200:
          description: "Successful operation; the resource permissions have been updated and the updated role is returned"
          content:
            application/json:
              schema:
                type: object
...
paths:
  /api/keys:
    get:
      tags:
      - "key"
      summary: "List top-level access keys"
      description: "Lists all top-level access keys which the requesting client has access to. The key itself is truncated so that only the last five characters are shown."
      responses:
        200:
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyTopLevelFormatted"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
    post:
      tags:
      - "key"
      summary: "Create a top-level access key"
      description: "Creates a new top-level access key, meaning it will not be tied to a client record. The access key will only have access to the resources it is explicitly granted."
      requestBody:
        content:
          application/json:
            schema:
              ...
              properties:
                description:
                  ...
                  description: A message describing the purpose of the access key
                resources:
                  description: A list of resources to assign to the key
                  $ref: "#/components/schemas/ResourceWithAccessMatrix"
      responses:
        200:
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyTopLevel"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/keys/{keyId}:
    delete:
      tags:
      - "key"
      summary: "Delete top-level access key"
      description: "Deletes a top-level access key identified by a given ID."
      parameters:
      - in: "path"
        name: "keyId"
        ...
        schema:
          type: string
        example: "5d7a716ee5f0fc52b1c13870"
      responses:
        204:
  ...
  /api/keys/{keyId}/resources:
    post:
      tags:
      - "key"
      summary: "Assign resource to top-level access key"
      description: "Assigns a resource to a top-level access key."
      parameters:
      - in: "path"
        name: "keyId"
        description: "The ID of the key to assign resources to"
        required: true
        schema:
          type: string
        example: "5d7a716ee5f0fc52b1c13870"
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyTopLevelFormattedWithResources"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/keys/{keyId}/resources/{resourceId}:
    put:
      tags:
      - "key"
      summary: "Update resource for top-level key"
      description: "Updates the permissions for a given resource on a top-level key."
      parameters:
      - in: "path"
        name: "keyId"
  ...
  /api/clients/{clientId}/keys:
    get:
      tags:
      - "key"
      summary: "List the access keys for a client"
      description: "Lists all top-level associated with a given client. The key itself is truncated so that only the last five characters are shown."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The client for which to list the access keys"
        required: true
        schema:
          type: string
        example: "eduardo"      
      responses:
        200:
          description: "List retrieved successfully successfully"
          content:
            application/json:
              schema:
                type: object
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClientFormatted"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
    post:
      tags:
      - "key"
      summary: "Create an access key for a client"
      description: "Creates a new access key associated with a client record. By default, the access key will have access to the same resources as the client it is associated with. If the access key is granted any resources, these will take precedence over any conflicting permissions coming from the client."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The client for which to create an access key"
        required: true
        schema:
          type: string
        example: "eduardo"
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClient"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/clients/{clientId}/keys/{keyId}:
    delete:
      tags:
      - "key"
      summary: "Delete client access key"
      description: "Deletes an access key associated with a client and identified by a given ID."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The client for which to delete an access key"
        required: true
        schema:
          type: string
        example: "eduardo"      
  ...
  /api/clients/{clientId}/keys/{keyId}/resources:
    post:
      tags:
      - "key"
      summary: "Assign resource to client access key"
      description: "Assigns a resource to a client access key."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The ID of the client to assign resources to"
        required: true
        schema:
          type: string
        example: "eduardo"
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClientFormattedWithResources"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/clients/{clientId}/keys/{keyId}/resources/{resourceId}:
    put:
      tags:
      - "key"
      summary: "Update resource for client access key"
      description: "Updates the permissions for a given resource on a client access key."
      parameters:
      - in: "path"
        name: "clientId"
        description: "The ID of the client to modify resources for"
        required: true
        schema:
          type: string
        example: "eduardo"
  ...
  /api/client/keys:
    get:
      tags:
      - "key"
      summary: "List the access keys for the requesting client"
      description: "Lists all top-level associated with the requesting client. The key itself is truncated so that only the last five characters are shown."
      responses:
        200:
          description: "List retrieved successfully successfully"
          content:
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClientSelfFormatted"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"  
    post:
      tags:
      - "key"
      summary: "Create an access key for the requesting client"
      description: "Creates a new access key associated with the requesting client. By default, the access key will have access to the same resources as the client it is associated with. If the access key is granted any resources, these will take precedence over any conflicting permissions coming from the client."
      requestBody:
        content:
          application/json:
            schema:
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClientSelf"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/client/keys/{keyId}:
    delete:
      ...
      - "key"
      summary: "Delete client access key"
      description: "Deletes an access key associated with the requesting client and identified by a given ID."
      parameters:
      - in: "path"
        name: "keyId"
        description: "The ID of the key to delete"
      ...
      responses:
        ...
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/client/keys/{keyId}/resources:
    post:
      tags:
      - "key"
      summary: "Assign resource to client access key"
      description: "Assigns a resource to an access key associated with the requesting client."
      parameters:
      - in: "path"
        name: "keyId"
        description: "The ID of the key to assign resources to"
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/AccessKeyClientSelfFormattedWithResources"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /api/client/keys/{keyId}/resources/{resourceId}:
    put:
      tags:
      - "key"
      summary: "Update resource for client access key"
      description: "Updates the permissions for a given resource on an access key associated with the requesting client."
      parameters:
      - in: "path"
        name: "keyId"
        description: "The ID of the key to update"
  ...
  schemas:
    AccessKeyClient:
      ...
      properties:
        ...
        token:
          type: string
          example: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhY2Nlc3NUeXBlIjoia2V5IiwiaWF0IjoxNTY4MzA1NTE4fQ.dkSf35pjlLVn2XjHrVbMKx_F9VtP2YAIqMkENMDAZzA"
        description:
          type: string
          example: React frontend
    AccessKeyClientFormatted:
      type: object
      properties:
        _id:
          type: string
        ...
        description:
          type: string
          example: React frontend
    AccessKeyClientFormattedWithResources:
      type: object
      properties:
        _id:
          type: string
    ...
    AccessKeyClientSelfFormatted:
      ...
      properties:
        ...
        description:
          type: string
          example: React frontend
    AccessKeyClientSelfFormattedWithResources:
      type: object
      properties:
        _id:
          type: string
    ...
    AccessKeyTopLevelFormatted:
      ...
      properties:
        ...
        description:
          type: string
          example: React frontend
    AccessKeyTopLevelFormattedWithResources:
      type: object
      properties:
        _id:
          type: string
...
- `resources` (type: `Object`, optional): An object containing resources and corresponding access matrices
...
The naming convention for collection specifications is `collection.<collection name>.json` where `<collection name>` is used asCcollectio namen/table in the underlying data store.
...
| enableVersioning | Whether to store a new document revision for each update/delete operation | `true` | `false`
| versioningCollection | Collection name used to hold revision documents | The collection name with the string `"Versions"` appended | `"authorsVersions"`
...
info:
  version: "1.0.0"
  title: DADI API - Collections API
  contact:
    email: eb@dadi.co
  license:
    name: Apache 2.0
...
paths:
  /api/collections:
    get:
      ...
      description: "Shows a list of all the existing collections and media buckets which the requesting client has access to."
      parameters:
      - name: "fields"
        in: "query"
        description: "Fields projection JSON string to be applied to the collection results"
        schema:
          type: string
        example: "{\"slug\":1,\"fields\":1}"
      responses:
        200:
          description: "Successful operation"
          content:
    ...
    post:
            ...
            schema:
              ...
              properties:
                ...
                property:
                  ...
                  required: true
                fields:
                  description: The collection fields as an object mapping field names to their settings
                  required: true
                  $ref: "#/components/schemas/CollectionFields"
                settings:
                  description: The collection settings
                  required: false
      ...
      responses:
        ...
        403:
          $ref: "#/components/responses/ForbiddenError"
        409:
          description: A collection already exists with the name and property supplied
  /api/collections/{property}/{name}:
    get:
      tags:
      - "collection"
      summary: "Retrieve collection schema"
      description: "Shows the schema for a specific collection."
      parameters:
      - name: "property"
        in: "path"
      ...
      responses:
        ...
        403:
          $ref: "#/components/responses/ForbiddenError"
        404:
          $ref: "#/components/responses/CollectionNotFoundError"
    delete:
      tags:
      - "collection"
      ...
      responses:
        ...
        404:
          $ref: "#/components/responses/CollectionNotFoundError"
  /api/collections/{property}/{name}/fields:
    post:
      tags:
      - "collection"
      summary: "Add fields to collection"
      ...
      responses:
        ...
        404:
          $ref: "#/components/responses/CollectionNotFoundError"
        409:
          description: The collection already contains a field with the name supplied
  /api/collections/{property}/{name}/fields/{field}:
    put:
      tags:
      - "collection"
      summary: "Update field in collection"
      description: "Updates a field in a collection."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      requestBody:
        content:
          application/json:
            description: An object containing the properties to be updated in the field schema.
            schema:
              type: object
              example:
                required: false
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/CollectionListItem2"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
    ...
    delete:
      tags:
      - "collection"
      summary: "Delete field from collection"
      description: "Deletes a field from a collection."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      responses:
              ...
              schema:
                ...
                properties:
                  results:
                    type: array
                    items:
                      $ref: "#/components/schemas/CollectionListItem3"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
        404:
          $ref: "#/components/responses/CollectionNotFoundError"
  /api/collections/{property}/{name}/settings:
    put:
      tags:
      - "collection"
      summary: "Update collection settings"
      description: "Updates one or more properties in the collection settings."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      requestBody:
        content:
          application/json:
            description: An object containing the properties to be updated in the collection settings.
            schema:
              type: object
              example:
                displayName: Books
...
components:
      ...
      properties:
        ...
        settings:
          ...
          example:
            ...
            cache: false
        fields:
          $ref: "#/components/schemas/CollectionFields2"
    CollectionListItem3:
      type: object
      properties:
        name:
        ...
        settings:
          ...
          example:
            ...
            cache: false
        fields:
          $ref: "#/components/schemas/CollectionFields3"
    CollectionSettings:
      type: object
      example:
        displayName: Books
  ...
  responses:
    ...
    UnauthorizedError:
      description: Access token is missing or invalid
    BadRequestError:
      description: The request payload is malformed or contains invalid or missing data
security:
  - bearerAuth: []
```
...
paths:
  /{property}/{collection}:
    get:
      ...
      description: "Returns all documents in the collection."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      - name: "compose"
        in: "query"
        description: "Whether to compose references to linked documents, showing resolved documents instead of IDs – `false` disables composition, `true` enables composition for one level of references, numeric values enable composition for X levels and the string `all` enables composition for all levels."
        schema:
          type:
            oneOf:
              - boolean
      ...
      - name: "count"
        in: "query"
        description: "The maximum number of documents to be retrieved in one page."
        schema:
          type: number
        example: 30
      - name: "fields"
        in: "query"
        description: "A MongoDB-style fields projection, limiting the fieldset of the returned documents."
        schema:
          type: string
        example: "{\"title\":1,\"year\":1}"
      - name: "filter"
        in: "query"
        description: "A MongoDB-style query object to filter candidate results by."
        schema:
          type: string
        example: "{\"year\":{\"$gt\":1855}}"
      - name: "page"
        in: "query"
      ...
      - name: "page"
        in: "query"
        description: "The sort direction to sort results by, mapping field names to either 1 or 0, which will sort results by that field in ascending or descending order, respectively."
        schema:
          type: number
        example: "{\"title\":1}"
      responses:
    ...
    post:
      ...
      description: "Creates one or more documents in the collection."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      requestBody:
        content:
          application/json:
            required: true
            description: An object representing the document to be created, or an array of objects if the client wishes to create multiple documents at once.
            schema:
              type:
                oneOf:
    ...
    put:
      ...
      description: "Updates any documents that match a specific query."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
            ...
            schema:
              ...
              properties:
                query:
                  required: true
                  type: object
                  description: A query object for determining which documents will be updated
                  example:
                    title: For Whom The Bell Tolls
                update:
                  required: true
                  type: object
                  description: The update object, containing the properties to be updated as well as their new values
                  example:
                    author: 5d669beef6cc99f62b86d5a1
                description:
                  type: string
                  description: A message describing the update operation
                  example: Setting author for For Whom The Bell Tolls
      responses:
        200:
          description: "The list of updated documents is returned"
          content:
            application/json:
              schema:
                type: object
    ...
    delete:
      ...
      description: "Deletes any documents that match a specific query."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
          ...
          application/json:
            schema:
              ...
              properties:
                ...
                description:
                  type: string
                  description: A message describing the delete operation
                  example: Deleting all books published before 1950
      responses:
        204:
          description: "The documents have been deleted; no data is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
  /{property}/{collection}/{documentId}:
    get:
      tags:
      - "document"
      ...
      - name: "documentId"
        ...
        schema:
          type: string
        example: 5d669beef6cc99f62b86d5f5
      - name: "compose"
        in: "query"
        description: "Whether to compose references to linked documents, showing resolved documents instead of IDs – `false` disables composition, `true` enables composition for one level of references, numeric values enable composition for X levels and the string `all` enables composition for all levels."
        schema:
      ...
      - name: "version"
        in: "query"
        description: "The ID of a specific – and possibly past – version of a document."
        schema:
          type: string
        example: 5d669beef6cc99f62b86d5b3
      responses:
        200:
        ...
        403:
          $ref: "#/components/responses/ForbiddenError"
        404:
          $ref: "#/components/responses/DocumentIDNotFoundError"
    put:
      tags:
      - "document"
      summary: "Update document"
      description: "Updates a documents with a specifc ID."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      responses:
        200:
          description: "The updated document is returned"
          content:
            application/json:
              schema:
                type: object
    ...
    delete:
      ...
      description: "Deletes a documents with a specifc ID."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
      ...
      responses:
        204:
          description: "The document has been deleted; no data is returned"
        401:
          $ref: "#/components/responses/UnauthorizedError"
        403:
          $ref: "#/components/responses/ForbiddenError"
        404:
          $ref: "#/components/responses/DocumentIDNotFoundError"
  /{property}/{collection}/{documentId}/versions:
    get:
      tags:
      - "document"
      summary: "List document versions"
      description: "Returns all past versions of a document with the ID specified."
      parameters:
      - name: "property"
        in: "path"
        description: "Property name"
...
components:
  schemas:
    ...
    DocumentResult:
      type: object
      example:
        _createdAt: 1567005678616
        _createdBy: eduardo
        _id: 5d669beef6cc99f62b86d5f5
        _lastModifiedAt: 1567006145557
        _lastModifiedBy: eduardo
        title: "For Whom The Bell Tolls"
        author: 5d669beef6cc99f62b86d5a1
        year: 1940
        genre: War Story
    DocumentResultsArray:
      type: array
      items:
        $ref: "#/components/schemas/DocumentResult"
    DocumentVersion:
  ...
  responses:
    BadRequestError:
      description: "The operation could not be completed because the data in the request body has generated one or more validation errors"
      content:
        application/json:
          schema:
            type: object
    ...
    DocumentIDNotFoundError:
      description: "No documents found with the given ID"
security:
  - bearerAuth: []
```
...
| `version` | The ID of a particular revision of a document to retrieve. Applicable only when retrieving a single document by ID and the collection being queried has document versioning enabled.| `null` | `version=5c54612ed10f781ca6fff604`
...
The `@dadi/api` NPM module exports a factory function, named `Model`, which receives Collection name and returns a model instance with the following methods available.
...
- `indexes` (type: `Array`): Array of indexes created, with each element being an object with a `collection` and `index` properties, which represent Collection name and the name of the field, respectively
...
| Path | Description | Environment variable | Default | Format
|:--|:--|:--|:--|:--
| `defaultBucket` | The name of the default media bucket | *N/A* | `mediaStore` | String
| `buckets` | The names of media buckets to be used | *N/A* | `` | Array
| `tokenSecret` | The secret key used to sign and verify tokens when uploading media | *N/A* | `catboat-beatific-drizzle` | String
| `tokenExpiresIn` | The duration a signed token is valid for. Expressed in seconds or a string describing a time span (https://github.com/zeit/ms). Eg: 60, \"2 days\", \"10h\", \"7d\" | *N/A* | `1h` | *
| `storage` | Determines the storage type for uploads | *N/A* | `disk` | `disk` or `s3`
| `basePath` | Sets the root directory for uploads | *N/A* | `workspace/media` | String
| `pathFormat` | Determines the format for the generation of subdirectories to store uploads | *N/A* | `date` | `none` or `date` or `datetime` or `sha1/4` or `sha1/5` or `sha1/8`
| `s3.accessKey` | The access key used to connect to an S3-compatible storage provider | `AWS_S3_ACCESS_KEY` | `` | String
| `s3.secretKey` | The secret key used to connect to an S3-compatible storage provider | `AWS_S3_SECRET_KEY` | `` | String
| `s3.bucketName` | The name of the S3 bucket in which to store uploads | `AWS_S3_BUCKET_NAME` | `` | String
| `s3.region` | The region for an S3-compatible storage provider | `AWS_S3_REGION` | `` | String
| `s3.endpoint` | The endpoint for an S3-compatible storage provider | *N/A* | `` | String
...
|-----------------|------------|---------------------------------------------|-------------------------------------------|-----------------------------------------------|
| mediaStore      | **Yes**    | `/media/mediaStore/count` or `/media/count` | `/media/mediaStore/sign` or `/media/sign` | `/media/mediaStore/upload` or `/media/upload` |
| otherBucket     | No         | `/media/otherBucket/count`                  | `/media/otherBucket/sign`                 | `/media/otherBucket/upload`                   |
| someOtherBucket | No         | `/media/someOtherBucket/count`              | `/media/someOtherBucket/sign`             | `/media/someOtherBucket/upload`               |
...
As most commands, uploading a file to API requires an authenticated request. In the case of file uploads, this can be done using a typical [oAuth bearer token](#obtaining-an-access-token) or a [pre-signed URL](#pre-signed-urls).
...
>
> It is possible to upload multiple files as part of the same request. DADI API will process all fields in a `multipart/form-data` request and treat each one as a separate file.
>
> -- advice

#### Custom metadata

When uploading a file, you can send additional metadata properties of your choice, such as caption, _alt text_ or copyright information. To do so, add a field with any name of your choice with a JSON-stringified object as its value. Please note that the name of the field is irrelevant in this case, it's the JSON payload that will be appended to the media object.
...
- `_id`: A hexidecimal string that uniquely identifies the newly-created media object
- `_createdAt`: The date and time of creation (UNIX timestamp)
- `_createdBy`: The name of the API client that uploaded the file
- `_version`: The version of the file uploaded (defaults to `1`)
- `contentLength`: The size of the file, in bytes
- `fileName`: The name of the file as it was uploaded
- `mimeType`: The MIME type of the file uploaded (e.g. `image/jpeg`)
- `path`: The path where the file was stored
...
- Original filename: `my trip to Portugal.jpg`
- New filename: `my_trip_to_Portugal.jpg`
...
Pre-signed URLs are useful to allow your users or applications to be able to upload a file without requiring an access token. When you request a pre-signed URL, you must provide an access token (see [Authentication](#authentication)) and specify an expected filename and MIME type for the file to be uploaded. The pre-signed URLs are valid only for the duration specified in the `media.tokenExpiresIn` configuration property (which can be [overrridden on a per-request basis](#override-the-expiry-when-requesting-a-signed-url)).

>
> It's currently not possible to upload multiple files simultaneously using a pre-signed URL. If you wish to upload multiple files, you can re-use the signed URL multiple times or you can bulk upload using an [access token](#obtaining-an-access-token).
>
> -- warning
...
### Referencing media from another collection
...
By default, versioning will be enabled on all collections. To disable it, set the `enableVersioning` property to `false` in the [collection settings block](#collection-settings).
...
### Specifying the versions collection
...
You can customise this name by setting the `versioningCollection` property in the [collection settings block](#collection-settings).
...
- `_id`: the ID of the document version
- `_document`: the original document ID
- `_changeDescription`: an optional message describing the operation that generated the version
...
|:----------------|:-------------------|:-------
| collection | Collection name that holds the reference data. Can be omitted if the field references data in the same collection as the referring document, or if the field references documents from multiple collections. | `"people"`
...
| `mediafieldv1` | Media field | 4.4.0
