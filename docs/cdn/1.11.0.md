...
* CDN will assume a default file extension of ".jpg" for any remote image requests that don't include an extension.
* If the URL for a remote image contains a querystring, CDN retains that querystring for use when retrieving the image. Any CDN image parameters in the URL are applied after the image is retrieved.
